...
data("support", package = "survival")
support <- support[support$ca %in% "metastatic", ]

# Create basic covariates

time  <- support$d.time
status <- support$death

# Diabetes (dummy)
diabetes <- model.matrix(~ factor(support$diabetes))[ , -1]

# Sex (dummy, female reference)
sex <- model.matrix(~ support$sex)[ , -1]

# Categorised age
age_cat <- cut(support$age,
               breaks = c(-Inf, 50, 60, 70, Inf),
               labels = c("<50", "50‑59", "60‑69", "70+"),
               right   = FALSE)

a_mat <- model.matrix(~ age_cat)[ , -1]

# Combine all predictors
z <- data.frame(a_mat, sex, diabetes)
colnames(z) <- c("age_<50", "age_50‑59", "age_60‑69", "age_70+", "male", "diabetes")

# Assemble full data frame
data_all <- data.frame(time = time,
                       status = status,
                       z)

# Split into external (historical), internal (current), and test sets
n  <- nrow(data_all)
...
idx_test <- idx[-c(1:(n_ext + n_int))]

external_data <- data_all[idx_ext, ]
internal_data <- data_all[idx_int, ]
test_data     <- data_all[idx_test, ]

# 1. Fit a Cox model on the external data to obtain coefficients
ext_cox <- coxph(Surv(time, status) ~ ., data = external_data)

beta_external <- coef(ext_cox)

# 2. Integrate external β into the internal data with varying eta
eta_seq <- seq(0, 5, by = 1)
result <- coxkl(z        = internal_data[ , names(beta_external)],
                delta    = internal_data$status,
                time     = internal_data$time,
                beta     = beta_external,
                eta_list = eta_seq)

# 3. Evaluate log‑partial likelihood on the held‑out test set
pred <- predict(result,
                newz  = test_data[ , names(beta_external)],
...
plot(pred,
     main = "Test Set Partial Likelihood vs. KL Weight (eta)",
     xlab = "eta (weight on external information)",
     ylab = "Partial log‑likelihood")
```
